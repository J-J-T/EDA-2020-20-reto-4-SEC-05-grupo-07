...
def req4(id,tiempo,citibike):
    ...
    respuesta=[]
    for i in citibike:
         print(i['start station id'])
         aentre=gr.getEdge(citibike["graph"], id, i['start station id'])
         if total+int(aentre['weight'])<tiempo:
            total=total+int(aentre['weight'])
            respuesta.append({'Nombre de estación inicio': i['start station id'], 'Nombre estación final':i['end station id'], 'Duración estimada del segmento':i['tripduration']})
    return respuesta
